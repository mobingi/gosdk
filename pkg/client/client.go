...
func (c *simpleHttpClient) Do(r *http.Request) (*Response, []byte, error) {
	if c.cnf.Verbose {
		if c.cnf.Logger == nil {
			simplelog.Info("[URL]", r.URL.String())
			simplelog.Info("[METHOD]", r.Method)
			for n, h := range r.Header {
				simplelog.Info(fmt.Sprintf("[REQUEST] %s: %s", n, h))
			}
		} else {
	...
	if c.cnf.Verbose {
		if c.cnf.Logger == nil {
			for n, h := range resp.Header {
				simplelog.Info(fmt.Sprintf("[RESPONSE] %s: %s", n, h))
			}

			simplelog.Info("[STATUS]", resp.Status)
		} else {
			for n, h := range resp.Header {
